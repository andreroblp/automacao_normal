...
+ '/ Sem Débito Automático', function () {
  ...
  it('Preenchendo Dados do Beneficiário', function () {
    ...
    cy.get('input[type="radio"]').check({ force: true }, 'false').should('be.checked')
  })
  ...
  it('Preencher Conferência e Revisão', function() {
    ...
    cy.get('[name="preBeneficiario.formaPagamentoMensalidade"]').select("Boleto");
  })

  it('Não pode existir a opção de Débito Automático', function() {
    cy.get('[name="preBeneficiario.formaPagamentoMensalidade"]').should('not.have.text', 'Débito Automático');
  })
  ...
  it('Impressão da Carteirinha', function (){
    cy.get('#avancar').click();
    cy.get('[data-icone="print"]').contains('Impressão da carteirinha').should('be.visible');
